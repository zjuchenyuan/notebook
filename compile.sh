...
sed -i 's#https://d.py3.io/btc.html#<div id="realtimeprofit"></div><script>fetch("https://d.py3.io/btc.html?_t="+new Date().getTime()).then(r=>r.text()).then(html=>document.getElementById("realtimeprofit").innerHTML=html)</script>#' docs/Bitcoin/index.html