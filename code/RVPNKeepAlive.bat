...
REM 由于rvpn会自动启动浏览器，可能导致大量内存占用，所以启动后自动结束浏览器进程

taskkill /f /im LogoutTimeOut.exe
if "%errorlevel%"=="0" taskkill /f /im SangforCSClient.exe&ping www.baidu.com&start "" "C:\Program Files\Sangfor\SSL\SangforCSClient\SangforCSClient.exe" /ShortCutAutoLogin&& taskkill /f /im 360Chrome.exe &ping www.baidu.com
